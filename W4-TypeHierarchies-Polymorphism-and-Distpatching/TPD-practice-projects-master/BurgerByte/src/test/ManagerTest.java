...
public class ManagerTest {
    ...
    public void testfire() {
        assertEquals(manager.getTeam().size(), 2);
        assertEquals(manager.getManagingBranch().getEmployee().size(), 2);
        manager.fire(frycook);
        assertEquals(manager.getTeam().size(), 1);
        assertEquals(manager.getManagingBranch().getEmployee().size(), 1);
        assertFalse(manager.getTeam().contains(frycook));
        assertFalse(manager.getManagingBranch().getEmployee().contains(frycook));
    }
    ...
    public void testhire() {
        ...
        assertEquals(manager.getManagingBranch().getEmployee().size(), 3);
    }
